...
const Navbar = () => {
  ...
  const isControlPanelActive = currentPath.includes("control-panel");

  const isInicioActive = !isHistoryActive && !isControlPanelActive;
  ...
  const handleItemClick = (item) => {
    switch (item) {
      ...
      case "history":
        switch (user.role) {
          case "AUTHORIZING_TWO_MODE_AUTH":
            navigate("/payer/documents-history");
            break;
          case "PAYER":
            navigate("/payer/documents-history");
          case "SUPPLIER":
            navigate("/supplier/documents-history");
            break;
          case "MANAGER":
            navigate("/admin/documents-history"); 
            break;
        }
        break;
      case "control-panel":
        navigate("/operator/control-panel");
    }
  };
      ...
      <div className="max-w-8xl mx-auto px-4">
        <div className="flex justify-between items-center">
          ...
          <nav className="flex items-center space-x-4">
            ...
            {["MANAGER", "PAYER", "SUPPLIER", "AUTHORIZING_TWO_MODE_AUTH"].includes(user?.role) && (
              <button
                type="button"
