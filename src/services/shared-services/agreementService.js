...
const updateDocumentsSelected = async(payload) => {
    ...
    try {
        ...
        const response = await api.post(
            `/agreement/updateDocuments/${payload.agreementId}/${payload.status}/${payload.payerId}/${payload.authMode}/${payload.userId}/${payload.termVersionId}`,
            requestData
        );
...
const getAgreements = async(user) => {
    ...
    console.log("ejecutando petición para tipo de entidad: ");
...
const approveDocumentsInBulk = async(payload) => {
    const response = await api.post('/agreement/bulk-approve', payload);
