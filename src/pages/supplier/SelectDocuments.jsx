...
import { disclaimerService } from "../../services/disclaimer/disclaimerService.js";
...
const SelectDocuments = () => {
  ...
  useEffect(() => {
    const loadTermVersion = async () => {
      try {
        const response = await auditService.getByActiveStatusSupplier();
  ...
  useEffect(() => {
    const loadEntity = async () => {
      try {
        // Asegúrate de que agreement.supplier no sea null o undefined antes de llamar al servicio
        if (!agreement?.supplier) return;

        const entity = await entityService.getEntityById(agreement.supplier);

        if (entity.status === 200) {
          setSupplierEntity(entity.data); // Esto se actualizará en el próximo render

          const disclaimerPayload = await disclaimerService.getDisclaimerBody(
            entity.data.id,
          );

          console.log("Respuesta real de la API:", disclaimerPayload);

          const datosDelDisclaimer = disclaimerPayload.data;

          if (datosDelDisclaimer) {
            setDisclaimerBody({
              entityId: datosDelDisclaimer.entityId,
              interestRate: datosDelDisclaimer.interestRate,
              benchmarkInterestRate: datosDelDisclaimer.benchmarkInterestRate,
              percentagePoints: datosDelDisclaimer.percentagePoints,
            });
          } else {
            console.warn(
              "No se encontró la propiedad 'data' en la respuesta",
              disclaimerPayload,
            );
          }
        }
  ...
  useEffect(() => {
    const loadNextDisbursementDate = async () => {

      if (!agreement?.payer) return;

      try {
        // Nota: Reemplaza 'operationsService.getNextDisbursementDate'
        // por la ruta exacta de tu capa de servicios en React.
        const response = await entityService.getNextDisbursementDate(
          agreement.payer,
        );
        ...
        if (response.status === 200 || response.data) {
          ...
          // 2. Convertimos el String de fecha ISO (YYYY-MM-DD) del backend a un Objeto Date de JavaScript
          const backendDate = new Date(data.nextDisbursementDate + "T00:00:00");

          // 3. Seteamos el estado para que impacte la interfaz
          setDisbursementDate(backendDate);

          console.log(
            `[Cerebro Financiero] Próxima fecha cargada: ${data.nextDisbursementDate} (${data.policyName})`,
          );
        }
      } catch (error) {
        console.error(
          "Error al obtener la próxima fecha de desembolso del backend:",
          error,
        );
        // Fallback: Si el endpoint falla, puedes llamar a tu función manual anterior como plan de respaldo
        calculateNextFriday();
      }
    };

    loadNextDisbursementDate();
  ...
  useEffect(() => {
    const total = accountsPayable.reduce(
      (sum, item) => (item.checked ? sum + (item.amount || 0) : sum),
      0,
    );
  ...
  const calculatePercentage = () => {
    ...
    const result =
      ((totalAmount - totalAmount * 0.0055 - totalInterests) / totalAmount) *
      100;
  ...
  const guardarDatos = async () => {
    try {
      ...
      // 1. Extraemos el ID de forma segura comprobando todas las posibles variantes
      const idConvenioSeguro =
        agreement.id || agreement.agreement_id || agreement.agreementId;

      if (!idConvenioSeguro) {
        console.error("El objeto agreement actual es:", agreement);
        throw new Error(
          "Se perdió la referencia al ID del convenio en el estado.",
        );
      }

      const payload = {
        agreementId: idConvenioSeguro, // Usamos la variable segura
        documentIds: selectedIds,
        status: DOCUMENT_STATUS.SELECTED,
        payerId: agreement.payer,
        userId: userData.id,
        authMode: AUTH_MODES.SINGLE_MODE,
        termVersionId: termVersion,
      };

      const response = await agreementService.updateDocumentsSelected(payload);

      // 2. En lugar de intentar extraer el ID generado de response.data (que podría venir vacío),
      // utilizamos el ID seguro que ya sabemos que es el correcto.
      const { data } =
        await agreementService.getAgreementById(idConvenioSeguro);
  ...
  const save = async () => {
    try {
      ...
      await guardarDatos();
      //await saveAcceptanceAudit();

      setAccountsPayable((prev) =>
        prev.map((item) => (item.checked ? { ...item, checked: false } : item)),
      );
  ...
  const NoDocumentsView = () => (
    <div className="flex flex-col items-center justify-center gap-4 text-gray-500 py-12">
      ...
      <p className="text-sm max-w-xs text-center">
        No hay documentos pendientes de selección en este momento. ¡Buen
        trabajo!
      </p>
    </div>
  );
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap w-40">
                        <span
                          className={`font-bold text-sm ${isDisabled ? "text-gray-400" : "text-red-950"}`}
                        >
                          {item.CCF}
                        </span>
                      </td>
                      ...
                      <td
                        className={`px-6 py-4 whitespace-nowrap text-sm w-32 ${isDisabled ? "text-red-600 font-semibold" : "text-gray-500"}`}
                      >
  ...
  // Aseguramos que tengamos números, si están vacíos usamos 0
  const interestRate = Number(disclaimerBody?.interestRate) || 0;
  const benchmarkRate = Number(disclaimerBody?.benchmarkInterestRate) || 0;
  const percentagePointsRate = Number(disclaimerBody?.percentagePoints || 0);

  // Función para separar y convertir a letras
  const formatRateToWords = (rate) => {
    // Aseguramos 2 decimales (ej: 12.50)
    const parts = rate.toFixed(2).split(".");
    return {
      enteroTexto: numeroALetras(parts[0]),
      decimalTexto: numeroALetras(parts[1]),
      numeroOriginal: rate.toFixed(2),
    };
  };

  const interestParts = formatRateToWords(interestRate);
  const benchmarkParts = formatRateToWords(benchmarkRate);
  const percentagePointsParts = formatRateToWords(percentagePointsRate);

  // Calcular el diferencial y convertir a letras
  const diferencialNum = Math.abs(interestRate - benchmarkRate).toFixed(2);
  const diferencialTexto = `${numeroALetras(diferencialNum.split(".")[0])} PUNTO ${numeroALetras(diferencialNum.split(".")[1])}`;
    ...
    <div className="flex min-h-screen gap-4 rounded-lg">
      <div className="w-4/6 flex flex-col">
        ...
        <div className="flex flex-col gap-4 justify-between mt-4">
          {step === 1 && <div className="h-full">{renderPayableItems()}</div>}
        ...
        <div className="">
          {step === 1 && (
            <button
              ...
              onClick={nextStep}
              className={`cursor-pointer mt-4 w-full py-2 px-4 rounded-md shadow-lg text-sm font-medium text-white ${
                accountsPayable.some((item) => item.checked)
      ...
      >
        ...
        <div className="text-gray-700 mb-6 max-h-72 overflow-y-auto pr-4 text-sm space-y-3">
          <p>
            ...
            siguientes Términos y Condiciones aplicables al Servicio Bancario
          ...
          <p>
            ...
            del <strong>{interestParts.enteroTexto}</strong> PUNTO{" "}
            <strong>{interestParts.decimalTexto}</strong> POR CIENTO y podrá
            ajustarse de manera quincenal a opción el Banco los días: uno y
            quince de cada uno de los meses comprendidos dentro del plazo y
            también de conformidad a la tasa de referencia que el banco
            mensualmente publica. La tasa de referencia correspondiente a este
            mes es del <strong>{benchmarkParts.enteroTexto}</strong> PUNTO{" "}
            <strong>{benchmarkParts.decimalTexto}</strong> por ciento, la que en
            sus publicaciones podrá ajustarse a opción del Banco; y el
            diferencial máximo que el banco podrá aplicar a este crédito durante
            toda su vigencia y mientras existan saldos pendientes será de{" "}
            <strong>{percentagePointsParts.enteroTexto}</strong> puntos
            porcentuales arriba de la tasa de referencia vigente a la fecha de
            cada modificación.
          </p>
          <p>
